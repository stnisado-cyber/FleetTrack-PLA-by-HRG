...
export default function FormPage({ cars, onSubmit }: Props) {
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4">
            {cars.map((car) => {
              ...
              return (
                <button key={car.id} disabled={!isAvailable} onClick={() => setFormData({...formData, carId: car.id})} className={`p-6 rounded-[2rem] border-2 text-left flex items-center justify-between transition-all ${isAvailable ? 'bg-white border-slate-100 hover:border-fuchsia-500' : 'bg-slate-50 border-slate-50 opacity-50 grayscale'}`}>
